...
(function ($) {
    ...
    var LG = 'linear-gradient(top, #fafafa, #eee)', CSS = '<style type="text/css">' +
        '#zxxBlank{position:absolute;z-index:2000;left:0;top:0;right:0;bottom:0;width:100%;height:100%;background:black;}' +
        '.wrap_out{width:95%;padding:2px;background:none;position:absolute;z-index:2000;left:-9999px;-webkit-border-radius: 8px; -moz-border-radius: 8px; -o-border-radius: 8px; border-radius: 8px;}' +
        '.wrap_in{background:none;}' +
        '.wrap_bar{background:#f0f0f0;background:-moz-' + LG + ";background:-o-" + LG + ";background:-webkit-" + LG + ";background:" + LG + ";}" +
        ...
        ".wrap_body{background: rgba(0, 0, 0, 0.5); color: #ffffff; border-radius: 5px; -webkit-border-radius: 5px; -moz-border-radius: 5px; -o-border-radius: 5px; word-break: break-all; word-wrap: break-word; }" +
        ".wrap_remind{width:10em;padding:10px;text-align:center;color:#ffffff;}" +
    ...
    $.fn.zxxbox = function (options) {
        ...
        return this.each(function () {
            if (this.nodeName.toLowerCase() === "a" && s.ajaxTagA) {
                $(this).click(function () {
    ...
    $.zxxbox = function (elements, options) {
        ...
        var s = $.extend(true, {}, zxxboxDefault, options || {});
        ...
        if (!$.o.s.padding) {
            ...
            if (typeof $.o.s.padding != 'number' || $.o.s.padding === true) {
                $.o.s.padding = "10px 15px";
            }
            $.o.bd.css('padding', $.o.s.padding || "10px 15px");
        }
        if ($.o.s.bd) {
            $.o.bd.css($.o.s.bd);
        }
    ...
    $.extend($.zxxbox, {
        setSize: function () {
            ...
            //主体内容的尺寸
            //console.log("width:"+$.o.s.width+",heigth:"+$.o.s.height);
            $.o.out.css({
                "width": $.o.s.width,
                "height:": $.o.s.height
            });
        ...
        setPosition: function (flag) {
            ...
            //console.log("{width: "+w+",height: "+h+", scrollTop: "+st+", body.height:"+ph+"}")
            if (ph < h) {
                ph = h;
            }
            ...
            //console.log("{outerHeight:"+xh+", outerWidth: "+xw+"}");
            var isOverflow = xw > w;
            if (isOverflow) {
                $.o.out.outerWidth(w);
            }
            var t = st + (h - xh) / 2, l = isOverflow ? 0 : (w - xw) / 2;
            if ($.o.s.fix && window.XMLHttpRequest) {
                t = (h - xh) / 2;
            }
            if ($.o.s.position) {
                t = $.o.s.position.top || t;
                l = $.o.s.position.left || l;
            }
            if (flag === true) {
                $.o.out.animate({top: t, left: l});
            } else {
                $.o.out.css({top: t, left: l, zIndex: $.o.s.index});
            }
            return $.o.out;
        },
        //定位
        ...
        loading: function () {
            ...
            return $.zxxbox(element, {bar: false, bg: false});
        },
        //ask询问方法
